...
This document describes what the devcontainer isolates, what it does not, and where gaps remain. The goal is to help users make informed decisions about their threat model rather than assume the container is a complete sandbox.
...
| Network | Open by default | Full egress; use `devc firewall on` to allowlist |
...
Containers have full outbound network access unless `devc firewall on` is used. Even with the firewall enabled:
...
`~/.gitconfig` is not mounted by default. If re-enabled (by uncommenting the volume in `docker-compose.yml`), the following information is exposed to the container:
...
Any path added via `devc mount` is writable by default. A compromised process inside the container can modify or delete files in mounted directories. Use `--readonly` when the container only needs read access.
...
Add default CPU, memory, and PID limits to `docker-compose.yml`:
