...
;;;
;;; This module is derived from guix/docker.scm of GNU Guix (commit
;;; a1d8d3837f, 2026-07-28), GPLv3+, with these additions to
;;; build-docker-image:
;;;
;;;   * #:user and #:working-dir, written to the image config as "User" and
;;;     "WorkingDir";
;;;   * #:owners, a list of (PATH UID GID): every file below PATH in the
;;;     non-store layer is archived with that owner instead of root.  Upstream
;;;     tars everything as 0:0, which leaves no way to give the agent user a
;;;     writable home directory;
;;;   * the non-store layer keeps the modes set by the populate directives
;;;     instead of normalizing them with --mode=go+u,go-w, so a 1777 /tmp
;;;     stays 1777.
;;;
...
(define-module (holocronix docker)
...
(define* (config layers-diff-ids time arch
                 #:key entry-point (environment '()) user working-dir)
  ...
  `((architecture . ,arch)
    (comment . "Generated by holocronix (guix)")
    (created . ,time)
    ...
    (history . ,(list->vector `(((created . ,time)
                                 (created_by . "holocronix jedicave-image")
                                 (comment . "holocronix")))))
    (os . "linux")
    (rootfs . ((type . "layers")
...
USER and WORKING-DIR, when true, are strings stored as the \"User\" and
\"WorkingDir\" of the image configuration.
...
OWNERS is a list of (PATH UID GID) tuples.  Files created by EXTRA-FILES at
or below PATH are archived with that owner; everything else is root-owned.
    ...
    (match-lambda
      (((head ...) (tail ...) id)
       ...
       (let* ((head-layers
         ...
         all-layers))))
  (define owned-paths
    ;; (PATH UID GID) with PATH relative, the way tar names members.
    (map (match-lambda
           ((path uid gid)
            (list (string-trim path #\/) uid gid)))
         owners))
  (define (archive-extra-layer top-level-entries store-paths)
    ;; Write ../layer.tar from the current directory.  Root-owned pass first,
    ;; skipping the owned subtrees, then one append pass per owned subtree.
    ;; Modes are kept as populated (no --mode), so directives decide them.
    (define base-options
      `("--mtime=@1" "--numeric-owner" "--sort=name"
        ,@transformation-options))
    (apply invoke "tar" "-cf" "../layer.tar"
           "--owner=0" "--group=0" "--anchored"
           `(,@base-options
             ,@(map (match-lambda
                      ((path _ _) (string-append "--exclude=" path)))
                    owned-paths)
             ,@top-level-entries
             ,@store-paths))
    (for-each (match-lambda
                ((path uid gid)
                 (when (file-exists? path)
                   (apply invoke "tar" "-rf" "../layer.tar"
                          (string-append "--owner=" (number->string uid))
                          (string-append "--group=" (number->string gid))
                          `(,@base-options ,path)))))
              owned-paths))
