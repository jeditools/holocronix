# Replace `workspace = true` values in a crate's Cargo.toml with the concrete
# values from its workspace root, so the crate can be used standalone from a
# cargo "directory" source (vendor directory).
#
# Implements the inheritance mechanism described at
# https://doc.rust-lang.org/cargo/reference/workspaces.html#the-package-table
#
# Adapted from nixpkgs (pkgs/build-support/rust/replace-workspace-values.py),
# MIT licensed.  Local change: prefer the stdlib tomllib when available.
#
# Usage: replace-workspace-values.py CRATE/Cargo.toml WORKSPACE-ROOT/Cargo.toml
...
def main() -> None:
    ...
    if "workspace" not in top_cargo_toml:
        ...
        # probably run on something that does not actually use workspace
        # dependencies
